// JavaScript til Universets Udvikling scene
let universeData = [];

// Hent Universets Udvikling data fra JSON
window.onload = () => {
  fetch("json/rumto.json")
    .then((response) => response.json())
    .then((data) => {
      universeData = data;
      updateUniverseEvolution();
    })
    .catch((error) => console.error("Fejl ved indlæsning af data:", error));

  // Opdater visningen af Universets Udvikling scenen
  function updateUniverseEvolution() {
    const phase = universeData[0];
    const header = document.getElementById("main-title");
    const speedSliderLabel = document.getElementById("speed-slider-label");

    if (!phase) {
      console.error(
        "Fejl: Data for Universets Udvikling scenen blev ikke hentet korrekt."
      );
      return;
    }

    if (header && speedSliderLabel) {
      header.textContent = phase.header;
      speedSliderLabel.textContent = phase.speedSlider.label;
    }
  }
  ...
  function addNewGalaxy() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const angle =
        (i / count) * Math.PI * 10 + armIndex * ((Math.PI * 2) / arms);
  ...
  function animate() {
    ...
    galaxies.forEach((galaxy) => {
      ...
      if (time > galaxy.delay) {
        ...
        galaxy.progress +=
          galaxyFormationSpeed * (actualTime + Math.random() * 0.05);
        if (galaxy.progress > 1) galaxy.progress = 1;

        for (let i = 0; i < particlePositions.length; i += 3) {
          particlePositions[i] =
            initialPositions[i] * (1 - galaxy.progress) +
            targetPositions[i] * galaxy.progress;
          particlePositions[i + 1] =
            initialPositions[i + 1] * (1 - galaxy.progress) +
            targetPositions[i + 1] * galaxy.progress;
          particlePositions[i + 2] =
            initialPositions[i + 2] * (1 - galaxy.progress) +
            targetPositions[i + 2] * galaxy.progress;
        }
      }
